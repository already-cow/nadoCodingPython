...
class Marine(AttackUnit):
    ...
    def stimpack(self):
        if self.hp > 10:
            ...
            print("{0} : 체력이 부족하여 스팀책을 사용하지 않습니다.".format(self.name))
    
# 탱크
class Tank(AttackUnit):
    #시즈모드 : 탱크를 시장에 고정시켜, 더 높은 파워로 공격 가능. 이동 불가
...
for unit in attack_units:
    if isinstance(unit, Marine): # 지금 만들어진 객체가 어떤 곳의 인스턴스인가 ?
        unit.stimpack()
...
# 전군 피해
for unit in attack_units:
    unit.damaged(randint(5, 21)) # 랜덤 공격
